...
void Ns::gen(unsigned int n,double bound1,int flag,mpz_class p,mpz_class *u,int *&I1,int *&I2,mpz_class *&U1,mpz_class *&U2,int &s1,int &s2,bool randomness_off){
    ...
    if(bound>=b1-a1){
       ...
       // }cout<< endl;
        if(!randomness_off){
            ...
            random_shuffle(&I1[1], &I1[n+1],random_function_test);
        }else{
    ...
    }else if(flag==2){//not used in this implementation
    ...
    // cout<<u[n]%p<<endl; //checking msb val
    // //print U1
    // for(unsigned long i=0;i<s1;i++){
    //     cout<<i<<" "<<u[I1[i]]<<" "<<I1[i]<<" "<< U1[i]<<endl;
    //     cout<<endl;
    // }cout<<endl;
    ...
    //     cout<<i<<" "<<u[I2[i]]<<" "<<I2[i]<<" "<< U2[i]<<endl;
    //     cout<<endl;
    // }cout<<endl;
}

/**
 * NSK attack function sets M1 and M2 sets calculate intersection and finds the msg if it exists
 */
