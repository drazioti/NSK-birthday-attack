...
// for parallel end
#include <gmpxx.h>//gmp lib c++ interface
...
int main(int argc, char **argv){
    ...
    // declare / init / set field
    srand(rand() ^ time(NULL));                         // initilize the seed
    //set nested parallelism
    // omp_set_dynamic(0);
    // omp_set_nested(1);
    
    unsigned int n;                                     // decalre var n biggest nat. t
...
so that (n/2-#) > 0\n\
 ...
 116 if p's bits are 2048 accordingly",{"sb","substruct"});
    ...
    cout << boolalpha;                                  // enable boolalpha flag to print value true as string true/false
    // set starting time var
    time_t clock_start = clock();
    //execute the NS attack & print result
    cout <<"Result : "<< ns->attack(n,bound,p,c,hamming,s,flag,I1,I2,U1,U2,s1,s2)<<endl; // perform the NS attack and print it's result
    //set & print timestamp
    cout << "Time : " << fixed << time_passed(clock_start) <<"s" << endl;    
